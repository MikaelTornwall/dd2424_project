...
def train_rbm(vecs, n_hidden, input_size, batch_size):
    ...
    for epoch in range(1, nb_epoch + 1): # for the sepcified number of epochs 
        for id_sent in range(0, n_sents - rbm_batch_size + 1, rbm_batch_size): # for each possible batch. Updated!
            vk = vecs[id_sent:id_sent+rbm_batch_size]
            v0 = vecs[id_sent:id_sent+rbm_batch_size]
...
def train_rbm_model_parameters(df, vector_set, batch_size):
    ...
    for hidden_layer in layers:
        ...
        rbm = train_rbm(visible_layer, hidden_layer, input_size, batch_size) # input size is the number of sentences
